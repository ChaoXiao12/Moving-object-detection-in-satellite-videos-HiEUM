...
if __name__ == '__main__':
    ...
    dis_th = [5]
    iou_th = [0.05]
    conf_thresh_all = [0.2,0.25,0.3]
    dataName = [3,5,2,8,10,6,9]

    data_dir = './datasets/RsCarData/'
    ANN_PATH0 = data_dir+'images/test1024/'
    eval_mode = 'fixed'
    th_mean = 1
    th_std = 13
    ...
    results_dir_tol = [
        './results/sp_centerDet_minus_trained_weights_model_best',
        ]
    ...
    for conf_thresh in conf_thresh_all:
        ...
        for results_dir0 in results_dir_tol:
            ...
            #####
            if eval_new_mode == 'new':
                txt_name1 = 'reuslts_%s_%.2f_pd_fa_new_gt.txt'%(eval_mode_metric, conf_thresh)
            else:
                txt_name1 = 'reuslts_%s_%.2f_pd_fa.txt' % (eval_mode_metric, conf_thresh)
            fid_pd = open(results_dir0 + txt_name1, 'w+')
            fid_pd.write(results_dir0 + '(pd,fa)\n')
            fid_pd.write(eval_mode_metric + '\n')
            #####
            ...
            for thre in thres:#
                ...
                for datafolder in dataName:
                    ...
                    if eval_mode == 'adaptive':
                        ...
                        results_dir = os.path.join(results_dir0, '%03d/' % (datafolder))
                    else:
                    ...
                    for index in range(num_images):
                        ...
                        #导入det
                        matname = os.path.join(results_dir, file_name.replace('.xml', '.mat'))
                        # matname = results_dir + 'mat/'+file_name.replace('.xml','.mat')
                ...
                #获取 avg results
                meatri = [[v['recall'], v['prec'], v['f1'], v['pd'], v['fa_1'], v['fa_2']] for k, v in results_temp.items()]
                ...
                fid_pd.write(
                    '&%.1f\t&%.1f\n' % (avg_results[3], avg_results[4]*1e6))
                results_temp['avg'] = {
