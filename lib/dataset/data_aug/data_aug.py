...
import sys
import os
from lib.dataset.data_aug.bbox_util import *

# lib_path = os.path.join(os.path.realpath("."), "data_aug")
# sys.path.append(lib_path)

def intersect(box_a, box_b):       #求交的区域
    max_xy = np.minimum(box_a[:, 2:], box_b[2:])
    min_xy = np.maximum(box_a[:, :2], box_b[:2])
    inter = np.clip((max_xy - min_xy), a_min=0, a_max=np.inf)
    return inter[:, 0] * inter[:, 1]

def jaccard_numpy(box_a, box_b):  #求交并比IoU
    ...
    """
    inter = intersect(box_a, box_b)             #求交
    area_a = ((box_a[:, 2]-box_a[:, 0]) *
              (box_a[:, 3]-box_a[:, 1]))  # [A,B]
    area_b = ((box_b[2]-box_b[0]) *
              (box_b[3]-box_b[1]))  # [A,B]
    union = area_a + area_b - inter             #求并
    return inter / union  # [A,B]

class RandomSampleCrop(object):#随机裁剪
    """Crop
    Arguments:
    ...
    Return:
        (img, boxes, classes)
            ...
            labels (Tensor): the class labels for each bbox
    """
    def __init__(self, image_size, crop_size, anno_coord = None):
        if anno_coord is None:
            self.crop_x1 = random.randint(1, image_size[0]-crop_size[0]+1)-1
            self.crop_y1 = random.randint(1, image_size[1]-crop_size[1]+1)-1
        else:
            w_x = random.randint(20, crop_size[0]-20)
            y_h = random.randint(20, crop_size[1]- 20)
            x1 = max(anno_coord[0]-w_x, 0)
            y1 = max(anno_coord[1]-y_h, 0)
            self.crop_x1 = int(min(x1, image_size[0]-crop_size[0]))
            self.crop_y1 = int(min(y1, image_size[0]-crop_size[0]))

        self.crop_x2 = int(crop_size[0]+self.crop_x1)
        self.crop_y2 = int(crop_size[1]+self.crop_y1)

        self.rect = np.array([self.crop_y1, self.crop_x1, self.crop_y2, self.crop_x2])

    def __call__(self, img, bboxes):
        img = img[self.crop_x1:self.crop_x2, self.crop_y1:self.crop_y2,:]
        bboxes_t = bboxes[:,:4].copy()
        overlap = jaccard_numpy(bboxes_t, self.rect)
        bboxes_select = bboxes[overlap>0, :]
        if bboxes_select.shape[0]>0:
            bboxes_select[:,0:4:2] -= self.crop_y1
            bboxes_select[:, 1:4:2] -= self.crop_x1

        return img, bboxes_select
...
class RandomTranslate(object):
    ...
    def __init__(self, translate = 0.2, diff = False):
        ...
        self.translate_factor_x = translate_factor_x
        self.translate_factor_y = translate_factor_y
...
class RandomRotate(object):
    ...
    def __init__(self, angle = 10):
        ...
        self.angle_out = random.uniform(*self.angle)
...
class RandomShear(object):
    ...
    def __init__(self, shear_factor = 0.2):
        ...
        self.shear_factor_out = random.uniform(*self.shear_factor)
        
    def __call__(self, img, bboxes):
    
        shear_factor = self.shear_factor_out
...
class RandomHSV(object):
    ...
    def __init__(self, hue = None, saturation = None, brightness = None):
        ...
        self.hue_out = random.randint(*self.hue)
        self.saturation_out = random.randint(*self.saturation)
        self.brightness_out = random.randint(*self.brightness)
    
    def __call__(self, img, bboxes):

        hue = self.hue_out
        saturation = self.saturation_out
        brightness = self.brightness_out
